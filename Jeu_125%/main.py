...
class FENETRE:
	...
	def start(self): # Contient la boucle qui permet l'affichage constant de la fenêtre et les interactions avec elle
			...
			background = pygame.transform.scale(BG, (int(bg_width) * 0.8, int(bg_height) * 0.8))

			# Chargement des écran de victoire
			winner_1 = pygame.image.load("Assets\winner_1.png")
			win_screen_1 = pygame.transform.scale(winner_1, (winner_1.get_width() * 0.8, winner_1.get_height() * 0.8))

			winner_2 = pygame.image.load("Assets\winner_2.png")
			win_screen_2 = pygame.transform.scale(winner_2, (winner_2.get_width() * 0.8, winner_2.get_height() * 0.8))
			...
			personnage_A = player.Perso(self.screen_size[0]//10, self.screen_size[1]//5, perso_A, 0.15, self.screen)

			perso_B = pygame.image.load("Assets/Perso_B.png")
			personnage_B = player.Perso(self.screen_size[0]//1.3, self.screen_size[1]//9, perso_B, 0.15, self.screen)
